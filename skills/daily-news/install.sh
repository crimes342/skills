...
function buildItem(r,cat){return{title:r.event.title,slug:r.event.slug,url:`https://polymarket.com/event/${r.event.slug}`,endDate:r.event.endDate,volume:r.event.volume,liquidity:r.event.liquidity,analysis:generateAnalysis(r.event),signals:r.signals};}
...
async function main() {
  ...
  const summary={scanTime:new Date().toISOString(),expiringCount:expiring.length,trendingCount:trending.length,anomalyCount:anom.length+trR.length,items:[...anom.map(r=>buildItem(r,'expiring_anomalies')),...trR.map(r=>buildItem(r,'trending_anomalies'))]};
